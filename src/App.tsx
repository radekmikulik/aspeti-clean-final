...
function AppContent() {
  ...
  // Dev test button - show when provider is loaded
  console.log('🔍 Rendering DevTestButtons:', { hasProvider: !!provider })
  const DevTestButtons = provider ? (
    <div className="fixed bottom-4 right-4 z-[9999] flex flex-col gap-2">
      <button
        ...
        onClick={(e) => {
          e.preventDefault()
          e.stopPropagation()
          console.log('🎯 Dev button clicked: Dashboard')
          handleNavigate('overview')
        }}
        className="px-3 py-2 bg-blue-600 text-white text-sm rounded-lg shadow-lg hover:bg-blue-700 cursor-pointer"
      ...
      <button
        ...
        onClick={(e) => {
          e.preventDefault()
          e.stopPropagation()
          console.log('🎯 Dev button clicked: Moje nabídky')
          handleNavigate('offers')
        }}
        className="px-3 py-2 bg-green-600 text-white text-sm rounded-lg shadow-lg hover:bg-green-700 cursor-pointer"
        style={{ pointerEvents: 'auto' }}
      >
        Test Moje nabídky
      </button>
  ...
  // Always show if provider exists
  if (!provider) {
    console.log('⚠️ No provider data, DevTestButtons not rendered')
  }
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && provider) {
      ...
      (window as any).debugOffers = () => {
        console.log('🎯 Manual debug: Activating OffersManagement')
        setShowOffersManagement(true)
        setCurrentPage('offers')
        setShowDashboard(false)
        setShowProfileEdit(false)
      ...
      (window as any).debugDashboard = () => {
        console.log('🎯 Manual debug: Activating Dashboard')
        setShowDashboard(true)
        setCurrentPage('offers')
        setShowProfileEdit(false)
        setShowOffersManagement(false)
      ...
      console.log('🔧 Debug functions available: window.debugOffers(), window.debugDashboard()')
    }
  }, [provider])
